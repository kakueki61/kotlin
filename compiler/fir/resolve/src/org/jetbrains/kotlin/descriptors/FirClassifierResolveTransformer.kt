...
class FirClassifierResolveTransformer : FirTransformer<Nothing?>() {
    ...
    override fun transformClass(klass: FirClass, data: Nothing?): CompositeTransformResult<FirDeclaration> {
        ...
        val typeParameters =
            klass.typeParameters.filterIsInstance<FirDescriptorOwner<*>>().mapNotNull { it.descriptor as? NewTypeParameterDescriptor }

        val nestedClassifiers =
            klass.declarations.filterIsInstance<FirDescriptorOwner<*>>().mapNotNull { it.descriptor as? NewClassifierDescriptor }

        val descriptor = NewClassDescriptorImpl(
            typeParameters,
